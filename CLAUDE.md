...
1. **请求流程**: 客户端 → Express服务器 → 协议转换 → 上游API → 响应转换 → 客户端
2. **配置热重载**: `config.json`变更会自动重载，源码变更需要重启
3. **Provider模式**: 每个上游服务商都有独立的协议转换器
4. **Web管理界面**: 前后端分离，通过REST API通信

### 常见开发任务

**添加新的上游服务商**:

1. 在`backend/src/providers/`创建新的provider实现
2. 在`backend/src/server.ts`中注册新provider
3. 更新类型定义和配置接口

**修改Web界面**:

1. 前端组件位于`frontend/src/components/`
2. API服务定义在`frontend/src/services/api.ts`
3. 使用Vuetify组件库保持界面一致性
4. API密钥管理功能在`AddChannelModal.vue`中，包含重复检测和高亮功能

**配置相关修改**:

1. 核心配置逻辑在`backend/src/config/config.ts`
2. 环境变量管理在`backend/src/config/env.ts`
3. 支持热重载，修改后无需重启服务器

### 调试和故障排除

- **开发模式**: 使用`bun run dev`获得详细日志和自动重启
- **健康检查**: 访问`/health`端点查看服务状态
- **配置验证**: 使用`bun run config show`检查当前配置
- **日志分析**: 开发模式提供请求/响应的详细日志

### 部署注意事项

- **环境变量**: 生产环境需要正确配置`.env`文件
- **SSL证书**: 支持跳过TLS验证选项(`insecureSkipVerify`)
- **进程管理**: 推荐使用PM2进行生产环境部署
- **监控**: 利用健康检查端点实现服务监控
