...
- **功能增强**:
  - **故障转移与降级**: 改进故障转移、日志记录和密钥降级策略
  - **上游请求超时与重试**: 为上游请求添加超时和重试机制，增强稳定性
  - **健康检查优化**: 优化健康检查逻辑，提供更准确的服务状态
- **Bug修复**:
  - **流式响应优化**: 修复流式响应中文本块的状态管理问题
  - **代理逻辑改进**: 完善请求代理逻辑，修复请求头和正文处理中的问题
- **代码重构**:
  - **JSON处理**: 优化内部JSON处理，提高序列化和反序列化性能

### 2025-10-12

- **添加API密钥复制功能**：在渠道卡片和编辑弹框中为每个密钥添加复制按钮
  - 支持一键复制完整密钥到剪贴板
  - 视觉反馈：复制成功后显示绿色勾选图标
  - 兼容性：现代浏览器使用Clipboard API，自动降级到传统方法
- **实现自动登录功能**：页面刷新时自动验证保存的访问密钥
  - 有效密钥自动登录，无需重复输入
  - 密钥失效自动提示用户重新输入
  - 显示加载提示"正在验证访问权限"
- **修复双重登录框问题**：移除Go后端内置HTML登录页面
  - 统一由前端Vue应用处理认证流程
  - 优化认证中间件逻辑
  - 解决页面加载时登录框闪烁问题
