...
- **Stream 错误处理完善**
  - 流式传输错误时向客户端发送 SSE 错误事件，避免连接挂起
  - 流式传输错误时记录失败指标，确保调度器感知坏 key/channel
  - 涉及文件：`handlers/common/stream.go`

- **CountTokens 端点安全加固**
  - 应用请求体大小限制，防止内存耗尽（DoS 风险）
  - 涉及文件：`handlers/messages/handler.go`

- **非 failover 错误指标记录**
  - 非重试错误（400/401/403 等）现在正确记录失败指标
  - 确保坏 key 健康度下降，避免持续参与轮询
  - 涉及文件：`handlers/messages/handler.go`、`handlers/responses/handler.go`
...
- **流量图表失败率可视化**
  - 失败率超过 10% 的时间段显示红色背景色带
  - 背景透明度随失败率动态变化（10%→0.08, 50%→0.35, 100%→0.65）
  - 自定义 Tooltip 显示每个 Key 的成功/失败详情及汇总统计
